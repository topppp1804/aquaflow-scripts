...
Config = {
    ...
    StuckLimit = 15,
    TravelLocation = "None",
    ShopCategory = "guns", -- New: guns or tubes
    ShopItem = "None"
}
...
-- [ QUEST PAGE CONTENT ]
local QuestNote = Instance.new("TextLabel", QuestPage)
QuestNote.Size = UDim2.new(1, -10, 0, 60); QuestNote.BackgroundTransparency = 1
QuestNote.Text = "QUEST AUTO-TP\nCollect items in TeleporterParts automatically based on Quest UI status."
QuestNote.TextColor3 = Color3.new(0.5, 0.5, 0.5); QuestNote.Font = Enum.Font.GothamMedium; QuestNote.TextSize = 9

local QuestBtn = Instance.new("TextButton", QuestPage)
QuestBtn.Size = UDim2.new(1, -10, 0, 45); QuestBtn.BackgroundColor3 = Color3.fromRGB(60, 60, 255)
QuestBtn.Text = "TP TO ACTIVE QUEST ITEM"; QuestBtn.TextColor3 = Color3.new(1, 1, 1); QuestBtn.Font = Enum.Font
    .GothamBold; QuestBtn.TextSize = 10
...
QuestBtn.MouseButton1Click:Connect(function()
    ...
    local questsUI = LocalPlayer:FindFirstChild("PlayerGui") and LocalPlayer.PlayerGui:FindFirstChild("Main") and
        LocalPlayer.PlayerGui.Main:FindFirstChild("RightCenter")
        and LocalPlayer.PlayerGui.Main.RightCenter:FindFirstChild("CanvasGroup") and
        LocalPlayer.PlayerGui.Main.RightCenter.CanvasGroup:FindFirstChild("Quests")

    if not partsRoot then
        QuestBtn.Text = "ERR: PARTS FOLDER MISSING"; task.wait(1); QuestBtn.Text = "TP TO QUEST ITEM"; return
    end

    local items = partsRoot:GetChildren()
    if #items == 0 then
        QuestBtn.Text = "ERR: 0 ITEMS IN FOLDER"; task.wait(1); QuestBtn.Text = "TP TO QUEST ITEM"; return
    end

    local targetItem, targetPart, skipCount = nil, nil, 0
    print("--- [ AquaFlow Quest Scan ] ---")
    for _, item in ipairs(items) do
        local isCompleted = false
        if questsUI then
            local qFrame = questsUI:FindFirstChild("Scientist_Find " .. item.Name) or questsUI:FindFirstChild(item.Name)
            if qFrame then isCompleted = isQuestDone(qFrame) end
        end
        if not isCompleted then
            local p = item:IsA("BasePart") and item or item:FindFirstChildWhichIsA("BasePart", true)
            if p then
                targetItem = item; targetPart = p; break
            end
        else
            print("[QUEST] Skipped: " .. item.Name); skipCount = skipCount + 1
        end
    end

    if targetItem and targetPart then
        QuestBtn.Text = "TP TO: " .. targetItem.Name:upper()
        if smoothTween then
            smoothTween(targetPart.CFrame + Vector3.new(0, 3, 0))
            QuestBtn.Text = "ARRIVED!"; task.wait(1); QuestBtn.Text = "TP TO NEXT"
        else
            QuestBtn.Text = "ERR: TWEEN NOT LOADED"
        end
    else
        QuestBtn.Text = string.format("DONE! (Found:%d Skip:%d)", #items, skipCount)
        task.wait(2); QuestBtn.Text = "TP TO QUEST ITEM"
    end
end)
    ...
    for _, f in ipairs(fishRoot:GetChildren()) do
        if isFishValid(f) and not Blacklist[f.Name] then
            ...
            if p then
                ...
                -- Strict dual filtering: Species AND Mutation
                local speciesMatch = Config.SelectedFish["All"] or Config.SelectedFish[fishName]
                local mutationMatch = Config.SelectedMutations["All"] or Config.SelectedMutations[mName]

                -- Mutation Only Mode: Force exclusion of Normal fish (Note: Shiny is treated as Normal)
                local isStrictMutation = (Config.TargetMode == "Mutation Only") and (mName == "Normal")
